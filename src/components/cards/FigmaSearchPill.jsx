...
const SEARCH_ARTICLE_LIMIT = 24;
const SEARCH_RESULT_LIMIT = 8;

function SearchSuggestions({ query, setQuery }) {
  /*
   * Cost saving:
   * This component mounts only when query has text.
   * So useArticles() does not run while search box is empty.
   */
  const articles = useArticles(SEARCH_ARTICLE_LIMIT) ?? [];
      ...
      {filtered.map((article, idx) => (
        <Link
          ...
          style={{
            ...
            background: "#F5F5F5",
            borderBottom:
              idx < filtered.length - 1 ? "1px solid #F1F1E6" : "none",
            transition: "background 0.2s",
          }}
          onMouseEnter={(e) => {
