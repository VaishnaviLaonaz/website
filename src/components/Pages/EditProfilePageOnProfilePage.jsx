...
//Correct also before cost cutting below one
...
export default function EditProfilePage() {
  ...
  const handleChange = ({ target }) => {
    ...
    if (["day", "month", "year"].includes(name)) {
      ...
      setForm((f) => ({
        ...
        roles: checked
          ? [...f.roles, value]
          : f.roles.filter((role) => role !== value),
      }));
    } else {
      setForm((f) => ({
        ...f,
  ...
  /* submit: compressed avatar upload, then save profile */
  const handleSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (imageUpload) {
        avatarUrl = await uploadAvatar(imageUpload, uid);
