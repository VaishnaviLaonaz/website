...
//above one is working before cost cutting
...
export function streamUnreadNotifications(userId, setCount) {
  ...
  return onSnapshot(
    ...
    (snap) => setCount(snap.size),
    (err) => {
      console.error("streamUnreadNotifications:", err);
      setCount(0);
    }
  );
}

/* ───────────── duplicate-safe notification ─────────── */
export async function tryNotify(userId, data) {
  try {
    if (!userId || !data?.type || !data?.actorId) return;
    ...
    const snap = await getDocs(duplicateQuery);

    if (!snap.empty) return;

    await addNotification(userId, data);
  } catch (err) {
    console.warn("notification skipped:", err?.message || err);
  }
}
 ...
 * Old behavior:
 * - Read all users
 * - Notify every user whenever one article is published
 *
 ...
 */
export async function notifyAllUsersArticlePublish() {
  return;
}